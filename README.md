...
`plan-delegation-decision.md` records both the recommended delegation depth and the actual authorized execution mode. The recommendation uses risk, scope, state/data integrity, verification complexity, and architecture tradeoffs as triggers; `.loopx/config.json` controls whether reaching the threshold may actually start subagent review. Missing delegation rationale blocks build handoff.
...
`config.json` records the loopx product contract plus init-time project discovery: existing AI rules such as `AGENTS.md` / `CLAUDE.md` / Cursor / Copilot files, existing spec sources such as `docs/changes` or ADR/RFC folders, and detected install/test/lint/typecheck/build/E2E commands. It also records `agent_delegation`, defaulting to disabled; use `loopx init --enable-agent-delegation --auto-agent-delegation --agent-delegation-threshold critic-only` or edit the config when the workflow is allowed to auto-start subagent review. This does not create a lighter loopx mode; it keeps project facts available to `plan`, `build`, and `review` while preserving loopx's full closed workflow.
...
- `.loopx/workflows/<slug>/plan-delegation-decision.md`: recorded recommended local/critic/parallel-review planning delegation mode, actual authorized mode, threshold, and authorization source.
...
- `architecture.md` is the architecture document. It covers system boundaries, component responsibilities, data/state models, interface integration, critical flows, quality attributes, and ADRs. It does not schedule implementation or define field-level details.
- `development-plan.md` is the development plan. It covers delivery slices, sequencing, dependencies, file-level change inventory, verification, human gates, rollback/degrade strategy, and done criteria. It does not choose the architecture.
- `.loopx/changes/active/<change-id>/design.md` is the detailed design. It maps requirements to concrete data structures, fields, API/function/component contracts, state-machine details, error handling, boundary cases, and test design. It is the closest planning input to build execution.

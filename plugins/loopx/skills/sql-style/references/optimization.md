...
2. Add indexes for demonstrated access paths, constraints, and frequent WHERE/JOIN patterns
3. Use covering indexes for frequent queries when read benefit justifies write and storage cost
4. Keep statistics up to date (ANALYZE regularly)
5. Avoid SELECT *, specify needed columns
6. Prefer EXISTS for existence checks; compare plans before replacing IN in dialect-sensitive queries

...
> 概要层内容**只完整存在一处**。当设计满足 design-review 的评审门（改公开契约/数据模型/状态机、跨系统、难回退、含需求方未见裁决）时，架构图、核心流程与模块划分由同目录的独立文档《概要设计.md》（design-review 产出）承载，本章只保留 3.1 方案总述与 3.6 专项设计检查，并在 3.1 指向该文档；3.2-3.5 整节删除。只有不过评审的设计才把 3.2-3.5 写在本章内。

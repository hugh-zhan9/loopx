...
`using-git-worktrees` is a support lens, not a workflow state. Use it before `exec` or manual implementation when isolation is needed. Do not use it to replace `plan2exec`, `exec`, `fix`, or `finish`.
